...
controller.createItem = async function(req, res) {
  try{
    //Procurando pela venda na qual o novo irem será inserido
    const venda = await Venda.findById(req.params.id)

    //se a venda não for encontrada, retorna 404: not found
    if(! venda) return res.status(404).end()

    //Adicionado o novo item ao vetor de itens da venda
    venda.itens.push(req.body)
    venda.markModified('itens')
    await venda.save()

    //em caso de sucesso, retorna http 201: Created
...
controller.retrieveOneItem = async function(req,res) {
  try{
    ...
    // encontra o item especifico dentro do vetor venda.itens
    const item = venda.itens.id(req.params.itemId)

    // se item for encontra, retorna-o com HTTP 200: OK (IMPLICITO)
    if(item) res.send(404).end()
  }
  catch(error) {
    console.error(error)
    // HTTP 500: Internal Server Error
